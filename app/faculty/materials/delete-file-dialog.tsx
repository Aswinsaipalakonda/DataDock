...
import { deleteMaterialFile } from "./actions";
...
export default function DeleteFileDialog({
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const res = await deleteMaterialFile(
        materialId,
        fileId,
        storageRef,
        targetLinkedIds
    ...
    <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 animate-in fade-in duration-200">
      ...
      >
        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            <h2 className="text-base font-bold text-slate-900">Delete Study File</h2>
            <p className="text-xs text-slate-500 font-normal">Permanent deletion of this resource</p>
          </div>
        </div>

        <p className="text-xs text-slate-600 mb-4 leading-relaxed">
          Are you sure you want to delete <span className="font-bold text-slate-900">&quot;{fileName}&quot;</span>? This will remove the file from enrolled students&apos; curriculum views.
        </p>
        ...
        {isMultiBranch && (
          <div className="mb-5 p-3.5 bg-slate-50 border border-slate-200/80 rounded-2xl space-y-2.5">
            <div className="flex items-center gap-2 text-xs font-bold text-slate-800">
              ...
              <span>Multi-Section Allocation Detected</span>
            </div>
            <p className="text-[11px] text-slate-500 font-normal">
              This unit is published to branches: <strong className="text-slate-800">{branches.join(", ")}</strong>. Where would you like to delete this file from?
            </p>
